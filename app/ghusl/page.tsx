...
const OBLIGATORY_CAUSES = [
  {
    n: 1,
    title: 'Janabah — Sexual Intercourse',
    arabic: 'الجنابة',
  ...
  {
    n: 3,
    title: 'End of Menstruation (Hayd)',
    arabic: 'انقطاع الحيض',
  ...
  {
    n: 4,
    title: 'Postnatal Bleeding (Nifas)',
...
const STEPS = [
  ...
  {
    step: 2,
    title: 'Bismillah and Washing Hands',
  ...
  {
    step: 5,
    title: 'Pour Water Over the Entire Body',
...
const WHAT_IT_PERMITS = [
  { item: 'Performing the five daily prayers (salah)' },
  { item: 'Reciting or touching the Quran' },
  { item: 'Entering and remaining in the mosque (masjid)' },
  { item: 'Performing tawaf around the Ka\'bah' },
  { item: 'Observing the obligatory fast (sawm)' },
  { item: 'Marital intimacy (after menstruation or nifas)' },
];
      ...
      <div className="max-w-4xl mx-auto px-6 py-14 space-y-16">
        ...
        <section>
          ...
          <p className="text-forest/70 leading-relaxed text-sm mb-6">
            Until ghusl is performed, a person in the state of janabah (major ritual impurity) is prohibited from prayer, reciting or touching the Quran, entering the mosque, or performing tawaf. The obligation is established firmly in the Quran:
          </p>
          <div className="card-forest rounded-2xl p-8">
            ...
            <p className="text-cream/80 text-sm mt-4 leading-relaxed">
              "And if you are in a state of janabah, then purify yourselves [by bathing your whole body]." (Al-Ma'idah 5:6)
            </p>
          </div>
        </section>
            ...
            {OBLIGATORY_CAUSES.map(cause => (
              <div key={cause.n} className="card-islamic flex gap-4">
                ...
                <div>
                  ...
                  <h3 className="font-garamond text-lg font-semibold text-forest mb-1">{cause.title}</h3>
        ...
        <section>
            ...
            {STEPS.map(step => (
              <div key={step.step} className="card-islamic flex gap-5">
                ...
                <div>
                  ...
                  <h3 className="font-garamond text-lg font-semibold text-forest mb-1">{step.title}</h3>
          ...
          <div className="gold-border-left pl-5 mt-8">
            <p className="text-forest/70 text-sm leading-relaxed">
              <strong>The minimum valid ghusl (fard elements):</strong> According to the majority of scholars, ghusl is valid at its minimum with: (1) intention, (2) rinsing the mouth, (3) rinsing the nose, and (4) pouring water over the entire body once so that no part of the skin remains dry. The full Sunnah method above is strongly recommended.
            </p>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            {WHAT_IT_PERMITS.map(p => (
              <div key={p.item} className="card-islamic flex items-start gap-3">
                <div className="w-2 h-2 rounded-full bg-gold mt-2 flex-shrink-0" />
                <p className="text-forest/70 text-sm leading-relaxed">{p.item}</p>
              </div>
            ))}
          </div>
        </section>
