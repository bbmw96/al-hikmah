...
import { QURAN_UI, SURAH_CONTENT, type QuranUiKey } from '@/lib/i18n/content/quran-content';

type Surah = (typeof SURAHS)[number];

export function SurahDetailContent({ surah }: { surah: Surah }) {
  const { lang } = useLanguage();
  const tc = (k: QuranUiKey) => QURAN_UI[k][lang] ?? QURAN_UI[k].en;
  const sc = SURAH_CONTENT[surah.number];
  const surahName = (s: Surah) => SURAH_CONTENT[s.number]?.name?.[lang] ?? s.englishName;
    ...
    <div className="max-w-4xl mx-auto px-6 py-12">
      ...
      <header className="relative bg-forest rounded-3xl overflow-hidden p-10 mb-10 text-center">
        <div className="relative z-10">
          <div className="flex items-center justify-center gap-3 mb-4">
            <span className="badge-gold text-xs">{tc('surah_label')} {surah.number}</span>
            <span className={cn(
      ...
      <div className="space-y-8">
        ...
        <section className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { label: tc('fact_verses'), value: String(surah.verses) },
            { label: tc('fact_revelation'), value: surah.classification },
            { label: tc('fact_period'), value: surah.period },
        ...
        <section className="card-islamic">
          ...
          <p className="text-forest/70 leading-relaxed">{sc?.overview?.[lang] ?? surah.overview}</p>
        </section>
        ...
        <section className="card-islamic border-gold/30 bg-gold/5">
          ...
          <p className="text-forest/70 leading-relaxed">{sc?.asbab?.[lang] ?? surah.asbabalNuzul}</p>
        </section>
        ...
        <section>
          ...
          <div className="flex flex-wrap gap-2">
            {surah.keyThemes.map((theme: string, i: number) => (
              <span key={theme} className="badge-gold text-xs">{sc?.themes?.[i]?.[lang] ?? theme}</span>
            ))}
          </div>
        </section>
        ...
        {surah.notableVerses.length > 0 && (
          <section>
            ...
            <div className="space-y-5">
              {surah.notableVerses.map((v: { reference: string; arabic: string; translation: string; context: string }, i: number) => (
        ...
        {surah.virtues && (
          <section className="card-islamic bg-forest text-cream">
            <h2 className="font-garamond text-xl font-semibold text-gold mb-3">{tc('heading_virtues')}</h2>
            <p className="text-cream/80 leading-relaxed text-sm">{sc?.virtues?.[lang] || surah.virtues}</p>
          </section>
        )}
      </div>
