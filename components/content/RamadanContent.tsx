...
function useRamadan() {
  ...
  return (key: RamadanContentKey): string => RAMADAN_CONTENT[key][lang as Lang];
}
...
const CONDITIONS_ITEMS = [
  { nameKey: 'cond_muslim' as RamadanContentKey, desc: 'Fasting is not obligatory upon non-Muslims, though they may choose to fast.' },
  { nameKey: 'cond_adult' as RamadanContentKey, desc: 'Children are not obligated but should be gradually encouraged as a preparation. Puberty marks the beginning of obligation.' },
  { nameKey: 'cond_sane' as RamadanContentKey, desc: 'One who lacks mental capacity is not obligated.' },
  { nameKey: 'cond_healthy' as RamadanContentKey, desc: 'One who is ill and for whom fasting would cause harm or delay recovery may break the fast and make it up later.' },
  { nameKey: 'cond_resident' as RamadanContentKey, desc: "A traveller covering a distance at which prayers are shortened may break the fast and make it up. Scholars differ on the distance threshold; the majority say approximately 80 km." },
  { nameKey: 'cond_capable' as RamadanContentKey, desc: 'A pregnant woman or nursing mother who fears harm to herself or her child may break the fast. The majority position is that she must make up the days, and some scholars also require fidyah (feeding a poor person) if the fast is broken out of fear for the child.' },
];

const INVALIDATORS = [
  { itemKey: 'inv_1' as RamadanContentKey, note: 'Forgetting and eating does not break the fast. The Prophet ﷺ said: "Whoever forgets and eats or drinks, let him complete his fast; Allah fed him and gave him to drink." (Bukhari 1933).' },
  { itemKey: 'inv_2' as RamadanContentKey, note: 'This requires making up the day and an expiation (kaffarah): freeing a slave, or if unable, fasting two consecutive months, or if unable, feeding 60 poor persons.' },
  { itemKey: 'inv_3' as RamadanContentKey, note: 'Vomiting involuntarily does not break the fast.' },
  { itemKey: 'inv_4' as RamadanContentKey, note: 'These break the fast even if they begin moments before sunset. The days must be made up.' },
  { itemKey: 'inv_5' as RamadanContentKey, note: 'Requires making up the day.' },
  { itemKey: 'inv_6' as RamadanContentKey, note: 'This terminates the fast along with all acts of worship.' },
];

const DOES_NOT_BREAK = [
...
const TARAWEEH = {
  ...
  positions: [
    {
      titleKey: 'taraweeh_8' as RamadanContentKey,
    ...
    {
      titleKey: 'taraweeh_20' as RamadanContentKey,
...
const SPIRITUAL_POINTS = [
  {
    titleKey: 'spiritual_1_title' as RamadanContentKey,
  ...
  {
    titleKey: 'spiritual_2_title' as RamadanContentKey,
  ...
  {
    titleKey: 'spiritual_3_title' as RamadanContentKey,
  ...
  {
    titleKey: 'spiritual_4_title' as RamadanContentKey,
...
export function RamadanContent() {
  ...
  const zakatRows: { labelKey: RamadanContentKey; value: string }[] = [
    { labelKey: 'zakat_label_amount', value: ZAKAT_FITR.amount },
    { labelKey: 'zakat_label_who_pays', value: ZAKAT_FITR.who_pays },
    { labelKey: 'zakat_label_who_receives', value: ZAKAT_FITR.who_receives },
    { labelKey: 'zakat_label_when', value: ZAKAT_FITR.when },
  ];
          ...
          <article className="card-islamic">
            ...
            <div className="bg-forest/5 rounded-xl p-4 space-y-3 mb-4">
              <p className="text-xs text-gold/70 uppercase tracking-wider font-medium">
                {tc('label_dua_iftar')} ({SUHUR_IFTAR.iftar.dua.source})
              </p>
            ...
            <div className="bg-gold/10 border border-gold/25 rounded-xl p-4 space-y-3">
              <p className="text-xs text-gold/70 uppercase tracking-wider font-medium">
                {tc('label_dua_alternative')} ({SUHUR_IFTAR.iftar.dua2.source})
              </p>
      ...
      <section>
        ...
        <div className="card-islamic mb-6">
          <p className="text-xs text-gold/70 uppercase tracking-wider font-medium mb-3">
            {tc('label_dua_laylatul_qadr')} ({LAYLATUL_QADR.dua.source})
          </p>
